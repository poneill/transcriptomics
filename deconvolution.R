...
#    sum(sapply(1:num.tests,function(i) 1-wilcox.test(xs,mu+sqrt(sigma.squared)*ns[i,] + alpha*as[i,]-beta*bs[i,])$p.value))
    sum(sapply(1:num.tests,function(i) ks.test(xs,mu+sqrt(sigma.squared)*ns[i,] + as[i,]/alpha-bs[i,]/beta)$statistic))
  }
...
recover.ab.dprime <- function(k3,k4,a.guess=1,b.guess=1,delta=0.01,epsilon=0.01){
  dfda <- function(a,b){12*(k3-2(1/b^3-1/a^3))/a^4 + 48*(k4-6*(1/b^4+1/a^4))/b^4}
  dfdb <- function(a,b){48*(k4-6*(1/b^4+1/a^4))/b^5 + 12*(k3-2*(1/a^3-1/b^3))/b^4}
  err1 <- function(a,b){
    k3 - 2*(1/a^3 - 1/b^3)
  }
  ...
  b <- b.guess
  grad <- c(-1,-1)
  while(abs(err.last - err.cur) > epsilon){
    grad <- c(dfda(a,b),dfdb(a,b))#normalize?
    a <- a - grad[1]*delta
    b <- b - grad[2]*delta
    err.last <- err.cur
